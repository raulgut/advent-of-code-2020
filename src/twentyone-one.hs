...
import Data.List (nub, find, intersect, (\\), partition)
...
main = do
  ...
  let nonAllergic = allIngredients \\ (concatMap snd solution)
  putStrLn . show . length . concatMap (filter (\x -> elem x nonAllergic) . fst) $ entries
