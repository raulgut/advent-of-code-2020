...
data PocketDimensionState =
  PocketDimensionState {
    grid :: Set (Int,Int,Int,Int)
    , gridNeighbours :: Map (Int,Int,Int,Int) [(Int,Int,Int,Int)]
  } deriving Show
...
main = do
  ...
  let entries = (concatMap (map (\(i,j,_) -> (i,j,0,0)) . filter (\(i,j,c) -> c == '#') . setPositions) . zip [0..] $ iLines) :: [(Int,Int,Int,Int)]
...
-- | get four-dimensional neighbours
neighbours :: (Int,Int,Int,Int) -> [(Int,Int,Int,Int)]
neighbours (x,y,z,w) = [(x', y', z', w') | x' <- [x - 1..x + 1], y' <- [y - 1..y + 1], z' <- [z - 1..z + 1], w' <- [w - 1..w + 1], not $ (x == x') && (y == y') && (z == z') && (w == w')]
...
tryNeighbours :: (Int, Int, Int, Int) -> State PocketDimensionState [(Int,Int,Int, Int)]
